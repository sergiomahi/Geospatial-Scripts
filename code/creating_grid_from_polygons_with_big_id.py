...
for index in grid_gdf.index:
    ...
    intersects_grid = gpd.GeoDataFrame({'big_grid_id':index, 'geometry':polygons})
...
for index, value in enumerate(intersects_grid_list):
    try:
        ...
        print(f'File name: {FILE_PREFIX}{intersects_grid_list[index].iloc[0].big_grid_id}.csv')
        tmp = intersects_grid_list[index]
        tmp['poly_id'] = intersects_grid_list[index].index
        tmp[['big_grid_id','poly_id', 'geometry']].to_csv(f'{FOLDER_PATH}/{FOLDER_NAME}/{FILE_PREFIX}{intersects_grid_list[index].iloc[0].big_grid_id}.csv', index=False)
end = time.time()
